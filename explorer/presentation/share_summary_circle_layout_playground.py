...
def _template_layouts_payload(template) -> dict[str, list[list[float]]]:
    positions = circle_card_template_positions(template)
    return {str(count): [[x, y] for x, y in coords] for count, coords in positions.items()}
