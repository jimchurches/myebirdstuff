...
**CI guardrail (main):** on **pull requests targeting `main`** and on **pushes to `main`**, the workflow runs `scripts/verify_explorer_build_not_behind_github_latest.py`, which fails if `explorer_build_version.txt` sorts **strictly older** than the latest GitHub release tag. This does **not** run for `beta-next`-only PRs so integration work is not blocked between releases.
