"""
Single source for checklist-style Streamlit HTML tab surface theme (green vs eBird blue).

Tab modules use :func:`inject_streamlit_checklist_css` for table + tab-surface injection, or
...
:data:`MAIN_TAB_STRIP_NAV_CSS`: muted inactive grey-green, primary green when selected,
...
:data:`MAIN_APP_HEADER_CSS`: dark green title and muted tagline aligned with the tab palette.
"""
...
from explorer.app.streamlit.defaults import (
    THEME_APP_HEADER_TITLE_HEX,
    THEME_MAIN_TAB_HOVER_HEX,
    THEME_MAIN_TAB_INACTIVE_MUTED_HEX,
    THEME_PRIMARY_HEX,
)
...
# brings Country / data tabs closer to the tab strip. Scoped to main column only; if Streamlit
...
section[data-testid="stMain"] [data-testid="stTabs"] button[role="tab"] {{
  color: {THEME_MAIN_TAB_INACTIVE_MUTED_HEX} !important;
}}
...
_MAIN_APP_HEADER_TAGLINE_HEX = THEME_MAIN_TAB_INACTIVE_MUTED_HEX
...
section[data-testid="stMain"] .pebird-app-header h1 {{
  color: {THEME_APP_HEADER_TITLE_HEX} !important;
...
def inject_main_tab_panel_top_compact_css() -> None:
    """Tighten tab panel inset and main tab strip styling.
...
def inject_streamlit_checklist_css(extra_css: str = "") -> None:
    """Inject scoped checklist table and Streamlit tab-surface CSS via ``st.html``.
