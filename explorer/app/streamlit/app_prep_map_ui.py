...
def _render_map_embed_body(
    *,
    ...
    Tags ``prep.map_iframe_embed`` events with the active ``embed_mode`` so JSONL splits cleanly
    when the ``EXPLORER_MAP_EMBED`` switch is exercised in #205 investigation A/Bs.
    """
    if map_hint_text:
