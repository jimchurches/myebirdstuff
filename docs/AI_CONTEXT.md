...
- **Ad-hoc HTML/CSS** — One-off `unsafe_allow_html` blobs **not** produced by a shared formatter are still a **conscious exception** (fragility, theming). Prefer routing table HTML through **`explorer/personal_ebird_explorer/`** helpers so formats stay aligned.
...
- When working on **any** area of the explorer, **bias toward** patterns that make a future Streamlit app easier:
  - Put **new or refactored logic** in `explorer/personal_ebird_explorer/` (or other testable modules) with **explicit inputs and return values**, not buried only in UI code.
