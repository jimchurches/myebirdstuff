...
| --- | --- | --- | --- | --- | --- | --- | --- |
| W1 | Wrap the Map-tab live embed in `@st.fragment` | Sidebar widget changes that don't affect map keys still trigger a full rerun, including a fresh `st_folium` render pass. A fragment isolates the map. | Removes 1 × `prep.map_iframe_embed` per off-map rerun. | `@st.fragment` interactions with the existing `_spinner_emoji_placeholder` and the dual-spinner story may be subtle. | Compare `prep.map_iframe_embed` count + total per "change non-map setting" journey. | A/B median across 3 runs on real CSV; recommendation comment on #205. | **dropped (batch 3)** — A/B (3 runs × 2 datasets × 2 modes via `EXPLORER_MAP_FRAGMENT={off,on}`) showed identical embed counts (6/6 events fixture, 6/6 real) and within-noise medians (real CSV: 6539 ms vs 6693 ms). Mechanism: `@st.fragment` only short-circuits when a widget _inside_ the fragment triggers a partial rerun; full-script `st.rerun()` from off-map widgets re-executes the whole script including the fragment, so the embed cost is paid every time. Recommendation in the [W1 A/B comment](https://github.com/jimchurches/myebirdstuff/issues/205#issuecomment-4414456363) is to drop the flag entirely; the cluster+popup parity test (I6) confirmed no visual regression in either mode. JSONL archives under [`benchmarks/map_perf/snapshots/issue-205-batch-3/w1-*.jsonl`](../../benchmarks/map_perf/snapshots/issue-205-batch-3); follow-up suggestion is **H2** (persistent client iframe with data-only updates), the only fragment-style isolation that survives full script reruns. |
