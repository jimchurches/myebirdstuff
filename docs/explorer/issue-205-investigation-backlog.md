...
| --- | --- | --- | --- | --- | --- | --- | --- |
| W1 | Wrap the Map-tab live embed in `@st.fragment` | Sidebar widget changes that don't affect map keys still trigger a full rerun, including a fresh `st_folium` render pass. A fragment isolates the map. | Removes 1 × `prep.map_iframe_embed` per off-map rerun. | `@st.fragment` interactions with the existing `_spinner_emoji_placeholder` and the dual-spinner story may be subtle. | Compare `prep.map_iframe_embed` count + total per "change non-map setting" journey. | A/B median across 3 runs on real CSV; recommendation comment on #205. | **batch 3** — implementation behind `EXPLORER_MAP_FRAGMENT={off,on}` flag (default `off`) in [`app_prep_map_ui.py`](../../explorer/app/streamlit/app_prep_map_ui.py). `prep.map_iframe_embed` perf events are tagged with `fragment_mode` so JSONL splits cleanly. New off-map rerun journey [`test_map_embed_off_map_rerun_journey_emits_perf_events`](../../tests/explorer/test_map_perf_e2e.py) drives the path the fragment is supposed to short-circuit (sidebar `Performance / debug` → `Clear session buffer` triggers `st.rerun()` without changing any `static_map_cache_key` component). I6 cluster + popup parity coverage shipped alongside (see I6 row). A/B measurement + recommendation pending in next sub-step. |
