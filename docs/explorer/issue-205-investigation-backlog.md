...
| W2 | Lite-popup A/B | Popup HTML assembly + Folium injection is a hidden cost in `prep.build_species_overlay_map` and `prep.folium_map_to_html_bytes`. | If popups dominate, a debug "lite HTML" mode should cut `popup_build_total_ms` and often `prep.folium_map_to_html_bytes` noticeably. | Product default must stay rich popups; lite mode is for **measurement** and future trade-off discussion—not a commitment to weaker UX. | Set `EXPLORER_MAP_LITE_POPUPS=1` (or Streamlit secret); compare JSONL / `aggregate_perf_jsonl` vs default. | A/B medians on real CSV; finding posted on #205 (kept / shelved / middle ground). | **implemented (batch 5); fixture A/B done (2026-05-12)** — flag + `lite_map_popups` on [`build_species_overlay_map`](../../explorer/core/map_controller.py) (wired from [`app_prep_map_ui.py`](../../explorer/app/streamlit/app_prep_map_ui.py); included in `static_map_cache_key` via `_render_opts_sig`; tagged in `prep.build_species_overlay_map` `extra`). **Lite preserves eBird:** every pin keeps the **location lifelist** link; **species-matching** pins add **one** checklist link (no full visit table, no species `<details>` sections). **Lifer** pins: lifelist only (species lines omitted—heaviest HTML path). Default remains unchanged. Tests: `test_lite_map_popups_*` in [`test_map_controller.py`](../../tests/explorer/test_map_controller.py); perf E2E + `EXPLORER_E2E_MAP_LITE_POPUPS` archive loop in [`test_map_perf_e2e.py`](../../tests/explorer/test_map_perf_e2e.py). **Fixture 3×2 A/B:** [`issue-205-w2-lite-ab-results.md`](issue-205-w2-lite-ab-results.md) (`popup_build_total_ms` median ~6.2→~0.1 ms; overlay build ~15.6→~9.3 ms; first paint unchanged on tiny CSV). **Real CSV** repeat optional for cold-load story. [Batch-4 baseline](https://github.com/jimchurches/myebirdstuff/issues/205#issuecomment-4416392292) motivated this. |
