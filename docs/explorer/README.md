...
**Streamlit (early prototype):** [`streamlit_app/`](../../streamlit_app/README.md) loads the same CSV (main-area upload on the landing page when disk resolution finds no file, or local path discovery) and shows a **Folium** map with **notebook-style location popups** (checklist links, visits) via `build_species_overlay_map`. It does **not** replace the notebook yet. Roadmap: [issue #70](https://github.com/jimchurches/myebirdstuff/issues/70). **Streamlit Community Cloud** must use **`requirements-streamlit.txt`** or **`streamlit_app/requirements.txt`** as the Python requirements file (not root `requirements.txt`). Prefer **native Streamlit** UI over custom HTML/CSS where practical — see [Streamlit UI guidelines](../../streamlit_app/README.md#ui-guidelines). When moving **rankings and other tables** from the notebook, **keep eBird links** (species / locations / checklists): many tallies already live on eBird; the app should stay connected. Streamlit dataframes don’t render Markdown in cells — see [AI_CONTEXT.md — Streamlit UI](../AI_CONTEXT.md#streamlit-ui-prefer-native-components).
