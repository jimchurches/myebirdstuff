...
def _expected_marker_cluster_tier_icon_hex(sch) -> tuple[str, ...] | None:
    """Mirror :func:`explorer.presentation.design_map_preview.scheme_seed_config` cluster hex resolution."""
    cl = sch.all_locations.cluster
    v = cl.tier_icon_hex
...
def _expected_cluster_inner_fill_opacity(sch) -> float:
    cl = sch.all_locations.cluster
    v = getattr(cl, "inner_fill_opacity", None)
    if v is None:
        return MAP_MARKER_CLUSTER_INNER_FILL_OPACITY_DEFAULT
    try:
